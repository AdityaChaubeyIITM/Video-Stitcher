...
def process_videos(video_path1, video_path2, output_path):
    ...
    while cap1.isOpened() and cap2.isOpened():
        ...
        if H is not None:
            # Warp the second frame using the homography matrix
            WarpImg = cv2.warpPerspective(frame2, H, (frame1.shape[1] * 2, frame1.shape[0] * 2))
            
            # Place the first frame onto the stitched canvas
            WarpImg[0:frame1.shape[0], 0:frame1.shape[1]] = frame1

            # Compute the ROI (region of interest) based on the bounds of the stitched area
            h, w, _ = frame1.shape
            corners = np.float32([[0, 0], [w, 0], [0, h], [w, h]]).reshape(-1, 1, 2)
            transformed_corners = cv2.perspectiveTransform(corners, H)
            
            x_min = int(min(0, min(transformed_corners[:, 0, 0])))
            y_min = int(min(0, min(transformed_corners[:, 0, 1])))
            x_max = int(max(frame1.shape[1], max(transformed_corners[:, 0, 0])))
            y_max = int(max(frame1.shape[0], max(transformed_corners[:, 0, 1])))

            # Adjust the coordinates to valid ranges
            x_min = max(0, x_min)
            y_min = max(0, y_min)
            x_max = min(WarpImg.shape[1], x_max)
            y_max = min(WarpImg.shape[0], y_max)

            # Crop the stitched image
            cropped = WarpImg[y_min:y_max, x_min:x_max]
    ...
    cap2.release()
    out.release()
    print("Stitched and cropped video saved to:", output_path)
